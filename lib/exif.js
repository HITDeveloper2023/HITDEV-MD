...
async function toGif(_0x1804d0) {
  try {
    const _0x10bdc6 = './' + randomBytes(0x3).toString("hex") + ".webp";
    const _0x59babe = './' + randomBytes(0x3).toString('hex') + ".gif";
...
async function toMp4(_0x1d5caf) {
  try {
    let _0x531e29 = './' + randomBytes(0x3).toString("hex") + ".gif";
    const _0x17da12 = fs.existsSync(_0x1d5caf) ? _0x1d5caf : save(_0x1d5caf, _0x531e29);
    const _0x222ca8 = './' + randomBytes(0x3).toString("hex") + '.mp4';
...
const EightD = async _0x78ba44 => {
  const _0x29f51a = "./temp/" + randomBytes(0x3).toString("hex") + '.mp3';
  _0x78ba44 = Buffer.isBuffer(_0x78ba44) ? save(_0x78ba44, _0x29f51a) : _0x78ba44;
  const _0x25915b = "./temp/" + randomBytes(0x3).toString('hex') + '.mp3';
  const _0x16290f = await new Promise(_0x4a7887 => {
    ffmpeg(_0x78ba44).audioFilter(["apulsator=hz=0.125"]).audioFrequency(0xac44).audioChannels(0x2).audioBitrate("128k").audioCodec("libmp3lame").audioQuality(0x5).toFormat("mp3").save(_0x25915b).on("end", () => _0x4a7887(_0x25915b));
  });
  return _0x16290f;
};
...
const _parseInput = async (_0x242492, _0x537749 = false, _0xbaa34d = "path") => {
  ...
  const _0x113b13 = "./temp/file_" + randomBytes(0x3).toString('hex') + '.' + (_0x537749 ? _0x537749 : (await fromBuffer(_0x257fef)).ext);
...
async function imageToWebp(_0x218725) {
  const _0x1e375a = path.join(tmpdir(), randomBytes(0x6).readUIntLE(0x0, 0x6).toString(0x24) + ".webp");
  const _0x23ff16 = path.join(tmpdir(), randomBytes(0x6).readUIntLE(0x0, 0x6).toString(0x24) + ".jpg");
...
async function videoToWebp(_0x359f57) {
  const _0x54fd5 = path.join(tmpdir(), randomBytes(0x6).readUIntLE(0x0, 0x6).toString(0x24) + ".webp");
  const _0x1a1e9b = path.join(tmpdir(), randomBytes(0x6).readUIntLE(0x0, 0x6).toString(0x24) + ".mp4");
...
async function writeExifImg(_0x1fd483, _0x483806) {
  ...
  const _0xef911d = path.join(tmpdir(), randomBytes(0x6).readUIntLE(0x0, 0x6).toString(0x24) + '.webp');
  const _0x3aa6e0 = path.join(tmpdir(), randomBytes(0x6).readUIntLE(0x0, 0x6).toString(0x24) + ".webp");
  fs.writeFileSync(_0xef911d, _0xb28c0b);
  if (_0x483806.packname || _0x483806.author) {
    const _0x335b84 = new webp.Image();
    const _0x42a636 = {
      'sticker-pack-id': "HITDEV-MD",
    ...
    const _0xcbb12e = Buffer.from([0x49, 0x49, 0x2a, 0x0, 0x8, 0x0, 0x0, 0x0, 0x1, 0x0, 0x41, 0x57, 0x7, 0x0, 0x0, 0x0, 0x0, 0x0, 0x16, 0x0, 0x0, 0x0]);
    const _0x6ed7c6 = Buffer.from(JSON.stringify(_0x42a636), "utf-8");
    const _0x306edf = Buffer.concat([_0xcbb12e, _0x6ed7c6]);
    _0x306edf.writeUIntLE(_0x6ed7c6.length, 0xe, 0x4);
...
async function writeExifVid(_0x45a241, _0x563630) {
  ...
  const _0x32aecd = path.join(tmpdir(), randomBytes(0x6).readUIntLE(0x0, 0x6).toString(0x24) + ".webp");
  const _0x5e9365 = path.join(tmpdir(), randomBytes(0x6).readUIntLE(0x0, 0x6).toString(0x24) + ".webp");
  ...
  const _0x33d1e4 = Buffer.from([0x49, 0x49, 0x2a, 0x0, 0x8, 0x0, 0x0, 0x0, 0x1, 0x0, 0x41, 0x57, 0x7, 0x0, 0x0, 0x0, 0x0, 0x0, 0x16, 0x0, 0x0, 0x0]);
  const _0x52afb6 = Buffer.from(JSON.stringify(_0x568dc0), "utf-8");
  const _0x5bdc63 = Buffer.concat([_0x33d1e4, _0x52afb6]);
  _0x5bdc63.writeUIntLE(_0x52afb6.length, 0xe, 0x4);
...
async function writeExifWebp(_0x38a80c, _0x8e963c) {
  const _0x182c66 = path.join(tmpdir(), randomBytes(0x6).readUIntLE(0x0, 0x6).toString(0x24) + '.webp');
  const _0xecd5c0 = path.join(tmpdir(), randomBytes(0x6).readUIntLE(0x0, 0x6).toString(0x24) + ".webp");
  fs.writeFileSync(_0x182c66, _0x38a80c);
  if (_0x8e963c.packname || _0x8e963c.author) {
    const _0x3f8a74 = new webp.Image();
    const _0x315c40 = {
      'sticker-pack-id': "Hitdev_Md",
    ...
    const _0x427b1b = await Buffer.from([0x49, 0x49, 0x2a, 0x0, 0x8, 0x0, 0x0, 0x0, 0x1, 0x0, 0x41, 0x57, 0x7, 0x0, 0x0, 0x0, 0x0, 0x0, 0x16, 0x0, 0x0, 0x0]);
    const _0xbfd185 = await Buffer.from(JSON.stringify(_0x315c40), 'utf-8');
    const _0x7b29c0 = await Buffer.concat([_0x427b1b, _0xbfd185]);
    await _0x7b29c0.writeUIntLE(_0xbfd185.length, 0xe, 0x4);
